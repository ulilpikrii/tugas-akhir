...
(function() {
  ...
  // WhatsApp Form Logic
  document.getElementById('waFormKonsultasi')?.addEventListener('submit', function(e) {
    e.preventDefault();
    ...
    const text = `Halo InfoPropertiLamongan, saya ingin konsultasi investasi.\n\nNama: ${name}\nEmail: ${email}\nTelepon: ${phone}\nAnggaran: ${budget}\nPesan: ${message}`;
    const url = `https://wa.me/6281234567890?text=${encodeURIComponent(text)}`;
    window.open(url, '_blank');
  });

  const footerWaForms = document.querySelectorAll('#footerWaForm');
  footerWaForms.forEach(form => {
